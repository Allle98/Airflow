...
PARQUET_FILE = os.getcwd() + os.sep + "dags" + os.sep + "tickers_exercise" + os.sep + "ticker_list.parquet"

BUCKET_NAME = "test_airflow_infomanager"
PROJECT_ID = "iungo-infomanager"
DATASET = "tickers_ds"
TABLE = "tickers_from_parquet"
...
SECRET_FILE = os.getcwd() + os.sep + "dags" + os.sep + "credentials" + os.sep + "iungo-infomanager.json"
                                                        ...
                                                        # daily
@dag(dag_id='ab_tickers_from_parquet', schedule_interval='@once', start_date=datetime(2022, 1, 1), tags=['iungo', 'gcs', 'bucket', 'tickers'], catchup=False)
def taskflow():

    @task
    def build_bq_file(filepath: str = PARQUET_FILE, start=(datetime.now() - timedelta(1)).strftime('%Y-%m-%d'), end=date.today()):
        ...
        # tickers = (pd.read_csv(filepath, sep=";"))
        tickers = pd.read_parquet(PARQUET_FILE)
        tickers = tickers.columns
        # print(tickers)
